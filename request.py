...
def test_login_invalid_credentials():
    response = requests.post(f'{BASE_URL}/login', json={'username': 'testuser', 'password': 'wrongpassword'})
...
def test_get_user_details_success():
    ...
    user_response = requests.get(f'{BASE_URL}/user/{user_id}', headers={'Authorization': f'Bearer {token}'})
...
def test_get_user_details_unauthorized():
    ...
    user_id = '12345'
    user_response = requests.get(f'{BASE_URL}/user/{user_id}')
